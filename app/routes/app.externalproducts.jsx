import { Card, Layout, Page, Text, Button, Thumbnail, Frame, Spinner, DataTable, Box } from '@shopify/polaris'
...
export default function ExternalProducts() {
  ...
  const rows = productsList.map((prod) => [
    <Thumbnail source={prod?.image || ''} alt={prod?.title || 'product image'}></Thumbnail>,
  ...
  const deleteProduct = () => {
    const filteredProducts = productsList?.filter((prod) => prod.id !== itemToDelete.id)
            ...
            <Card>
              ...
              {fetcher.state === 'loading' ? (
                ...
                <DataTable
                  ...
                  headings={['Image', 'Title', 'Description', 'Price', 'Edit', 'Delete']}
                  rows={rows}
                  stickyHeader
                  fixedFirstColumns={1}
